    ...
    static async haalKlantIdOpBackend(postcode, huisnummer) {
        ...
        try {
            ...
            const data = await client.db(mongoDBBackend)
                ...
                .toArray()
            if (data.length > 0) {
                console.log(`Klant gevonden met id: ${data[0]._id}`)
                return data[0]._id
            } else {
                console.log(`Geen klant gevonden voor de volgende query: ${JSON.stringify(query)} in database ${mongoDBBackend} and collection: ${collection}`)
                return null
            }
        } catch (err) {
            throw new Error (err.message)
